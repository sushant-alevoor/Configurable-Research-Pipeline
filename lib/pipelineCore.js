...
/* ============================================================
   STAGE 2.5 — PAYMENTS/UX DETERMINISTIC CAP (finance variant only)
   Deterministic post-curation filter, same philosophy as the dedup and
   real-URL-verification filters above: no LLM call, just an array slice.
   No-op for any profile that doesn't define laneConfig.paymentsUxCap
   (e.g. organizational-friction), so this is purely additive and safe
   to leave in pipelineCore.js for all profiles.
============================================================ */
export function applyPaymentsUxCap(laneConfig, curated) {
  const cap = laneConfig.paymentsUxCap;
  if (cap === undefined || cap === null) return curated;

  const label = `Curator (${laneConfig.label})`;
  const included = curated.filter(r => r.decision === 'Include');
  const notIncluded = curated.filter(r => r.decision !== 'Include');

  const core = included.filter(r => r.category !== 'payments_ux');
  let paymentsUx = included.filter(r => r.category === 'payments_ux');

  const before = paymentsUx.length;
  if (before > cap) {
    paymentsUx = paymentsUx
      .slice()
      .sort((a, b) => (b.score || 0) - (a.score || 0))
      .slice(0, cap);
    log(`${label}: capped payments/UX articles at ${cap} — trimmed ${before - cap}`, 'warn');
  }

  // Recombine so downstream stages (matcher, tables, Issue body) see a
  // normal Include list, unaware of the category distinction — per spec
  // Section 5.4, the rest of the pipeline stays untouched by this.
  return [...notIncluded, ...core, ...paymentsUx];
}
