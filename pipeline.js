...
import { log, isPaywalled } from './lib/util.js';
import { buildDedupSeenSet, loadVaultFile, tryParseDate } from './lib/dedup.js';
import { callLLM, runSearch } from './lib/providerRouter.js';
import { parseJsonArrayLenient } from './lib/parseJson.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

/* ============================================================
   CONFIG LOADING
   Pass a profile name via CLI arg or CONFIG_PROFILE env var, e.g.:
     node pipeline.js frictionless-architecture
     CONFIG_PROFILE=frictionless-architecture node pipeline.js
   No profile name and no keywords/prompts are hardcoded here — this
   script is topic-agnostic by design. See README for adding a new one.
============================================================ */
function loadConfig() {
  const profileName = process.argv[2] || process.env.CONFIG_PROFILE;
  if (!profileName) {
    throw new Error('No config profile given. Usage: node pipeline.js <profile-name> (matches a file in config/<profile-name>.json)');
  }
...
/* ============================================================
   STAGE 1 — SEARCH
============================================================ */
async function runSearchLane(laneKey, laneConfig, dedupSeenUrls) {
  const label = `Search (${laneConfig.label})`;
  const searchProvider = laneConfig.searchProvider || 'claude';
  let results;

  if (searchProvider === 'mock') {
    // Reads from a small local fixture instead of hitting any real search
    // API — for isolating curator/matcher/provider testing (e.g. verifying
    // a new LLM provider works) without spending real search quota.
...
/* ============================================================
   STAGE 2 — CURATE
============================================================ */
async function runCuratorLane(laneConfig, searchResults) {
  const label = `Curator (${laneConfig.label})`;
  log(`${label}: evaluating ${searchResults.length} articles against criteria`, 'info');

  if (searchResults.length === 0) return [];

  // Batch into chunks so a single response never has to carry more than
  // ~20 articles' worth of JSON — avoids truncation on large search
  // result counts, and keeps each call's output comfortably inside
  // maxTokens regardless of provider.
...
Return a JSON array with one object per article (same order), each with: id, decision, score, reason, tier.`;
  ...
  const hardLimitMonths = laneConfig.hardAgeLimitMonths || 0;
  if (hardLimitMonths > 0) {
    const cutoff = new Date();
    cutoff.setMonth(cutoff.getMonth() - hardLimitMonths);
    let backstopped = 0;
    curated = curated.map(r => {
      const d = tryParseDate(r.date);
      if (d && d < cutoff && r.decision !== 'Exclude') {
        backstopped++;
        return { ...r, decision: 'Exclude', reason: `${r.reason} (recency backstop: older than ${hardLimitMonths}mo hard limit)` };
      }
      return r;
    });
    if (backstopped > 0) log(`${label}: recency backstop force-excluded ${backstopped} article(s) older than ${hardLimitMonths}mo`, 'warn');
  }
...
/* ============================================================
   STAGE 3 — MATCH
============================================================ */
async function runMatcher(matcherPrompt, matcherLlmProvider, opInclude, csInclude) {
...
function buildMarkdownTables({ matches, opInclude, csInclude, opCurated, csCurated, opMax, csMax }) {
  ...
  // Only accept a proposed match if BOTH ids genuinely belong to this
  // lane's Include set — not just "found somewhere in curated" (which
  // would also match Flagged/Excluded articles the matcher shouldn't have
  // referenced, e.g. from a hallucinated/mis-copied id under low-thinking
  // settings). Anything that doesn't validate is dropped and logged rather
  // than silently corrupting the unmatched counts.
...
/* ============================================================
   MAIN
   This writes a DRAFT output file — it does NOT touch the Obsidian
   vault directly. Reviewing the draft and appending it into the vault
   is a deliberate manual step (the human-in-the-loop checkpoint),
   consistent with how the original artifact required manual copy/paste.
============================================================ */
async function main() {
  const { config, profileName } = loadConfig();
  log(`Pipeline started — profile: ${profileName}`, 'info');
